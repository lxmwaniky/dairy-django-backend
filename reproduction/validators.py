...
class PregnancyValidator:
    ...
    def validate_age(age, start_date, cow):
        ...
        if age < 365:
            raise ValidationError(
                ...
                f"This cow is {round(age / 30.417, 2)} months old.", code="age_below_threshold"
            )
        if not start_date:
            raise ValidationError("Provide pregnancy start date.", code="missing_start_date")
    ...
    def validate_cow_current_pregnancy_status(cow):
        ...
        if cow.current_pregnancy_status == CowPregnancyChoices.PREGNANT:
            raise ValidationError("This cow is already pregnant!", code="cow_already_pregnant")
        if cow.current_pregnancy_status == CowPregnancyChoices.CALVED:
            raise ValidationError("This cow just gave birth recently!", code="cow_calved_recently")
    ...
    def validate_cow_availability_status(cow):
        ...
        if cow.availability_status == CowAvailabilityChoices.DEAD:
            raise ValidationError("Cannot add pregnancy record for a dead cow.", code="dead_cow")

        if cow.availability_status == CowAvailabilityChoices.SOLD:
            raise ValidationError("Cannot add pregnancy record for a sold cow.", code="sold_cow")

    @staticmethod
    def validate_pregnancy_status(pregnancy_status, start_sate, pregnancy_failed_date, pregnancy_duration):
        """
        ...
        if (
            ...
            raise ValidationError(
                "Pregnancy is marked as failed, provide the date of failure", code="missing_date_of_failure"
            )
        ...
        if pregnancy_duration >= 30 and pregnancy_status not in [PregnancyStatusChoices.CONFIRMED,
                                                                 PregnancyStatusChoices.FAILED]:
            raise ValidationError(f"Pregnancy status must be confirmed or failed. "
    ...
    def validate_dates(start_date, pregnancy_status, date_of_calving, pregnancy_scan_date, pregnancy_failed_date):
        ...
        if start_date > todays_date:
            raise ValidationError("Start date cannot be in the future.", code="invalid_start_date")

        if date_of_calving and start_date:
            if date_of_calving < start_date:
                raise ValidationError("Date of calving must be after the start date.", code="invalid_date_of_calving")

            if date_of_calving > todays_date:
                raise ValidationError("Calving date cannot be in the future.", code="invalid_date_of_calving")
    ...
    @staticmethod
    def validate_failed_date_and_start_date(pregnancy_failed_date, start_date, pregnancy_status):
        """
    ...
    def validate_outcome(pregnancy_outcome, pregnancy_status, date_of_calving):
        ...
        if date_of_calving and pregnancy_outcome not in [
            ...
            raise ValidationError("Provide the pregnancy outcome", code="missing_outcome")
